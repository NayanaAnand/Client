...
PREPARE_LOGGING(Client_i)

Client_i::Client_i(const char *uuid, const char *label) :
    Client_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void Client_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &Client_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &Client_i::<callback method>)
        in the constructor.
        ...
        Client_i::Client_i(const char *uuid, const char *label) :
            Client_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &Client_i::scaleChanged);
            addPropertyListener(status, this, &Client_i::statusChanged);
        }

        void Client_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void Client_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
